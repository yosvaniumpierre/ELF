namespace Avanade.Platform.Services.Web.Configuration.DataSource
{
    ...
    internal class ExternalDefaultIniConfig : AbstractKeyConfigHandler
    {
        #region Fields
        ...
        private ExternalDefaultIniConfig(string externalFileConfig)
        {
        ...
        private ExternalDefaultIniConfig(string externalFileConfig, IDictionary<string, IConfigSettingCollection> settingsGroups)
        ...
        public static ExternalDefaultIniConfig Create(FileInfo fileInfo)
        {
            return fileInfo.Exists ? new ExternalDefaultIniConfig(fileInfo.FullName, IniFileProcessor.Load(fileInfo)) :
                new ExternalDefaultIniConfig(fileInfo.FullName);
        }
        ...
        {
            Logger.Info("Performing reinitialisation for the external default-section ini config file...");
        ...
        protected override bool HasKey(string key)
        {
            ...
            var containsSection = SettingGroups.ContainsKey(DefaultSection);
            var containsKey = false;

            if (containsSection)
            {
                containsKey = SettingGroups[DefaultSection].ContainsKey(key);
            }
